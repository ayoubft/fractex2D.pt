...
class DepthwiseSeparableConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size,
                 padding, bias=True):
        super().__init__()
        # The depthwise conv is basically just a grouped convolution in
        # PyTorch with the number of distinct groups being the same as
        # the number of input (and output) channels for that layer.
        self.depthwise = nn.Conv2d(
            in_channels, in_channels, kernel_size=kernel_size,
            padding=padding, bias=bias, groups=in_channels)
        # The pointwise convolution stretches across all the output channels
        # using a 1x1 kernel.
    ...
    def __init__(self, chin, chout, kernel_size):
        ...
        self.seq = nn.Sequential(
            DepthwiseSeparableConv2d(
                in_channels=chin, out_channels=chout, kernel_size=kernel_size,
                padding=kernel_size//2, bias=False),
            nn.BatchNorm2d(chout),
            nn.ReLU(),
            DepthwiseSeparableConv2d(
                in_channels=chout, out_channels=chout, kernel_size=kernel_size,
                padding=kernel_size//2, bias=False),
            nn.BatchNorm2d(chout),
            nn.ReLU(),
        )
...
class UpDSConv2(nn.Module):
    def __init__(self, chin, chout, kernel_size):
        ...
        self.seq = nn.Sequential(
            DepthwiseSeparableConv2d(
                in_channels=chin, out_channels=chin, kernel_size=kernel_size,
                padding=kernel_size//2, bias=False),
            nn.BatchNorm2d(chin),
            nn.ReLU(),
            DepthwiseSeparableConv2d(
...
class SegNet(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size):
        ...
        self.uc5 = UpDSConv3(512, 512, kernel_size=kernel_size)
