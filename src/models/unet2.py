...
class UNet(nn.Module):
    def __init__(self, in_channels=4, out_channels=1,
                 init_features=32, dropout=False):
    ...
    def _block(in_channels, features, name, dropout=False):
        layers = [
            (name + "conv1", nn.Conv2d(in_channels, features,
                                       kernel_size=3, padding=1, bias=False)),
            (name + "norm1", nn.BatchNorm2d(features)),
            (name + "relu1", nn.ReLU(inplace=True)),
            (name + "conv2", nn.Conv2d(features, features,
                                       kernel_size=3, padding=1, bias=False)),
            (name + "norm2", nn.BatchNorm2d(features)),
            (name + "relu2", nn.ReLU(inplace=True)),
        ]
        if dropout:
            layers.append((name + "dropout", nn.Dropout()))
