...
Original code is from the following source. It comes with MIT License
...
def _window_2D(window_size, power=2):
    ...
    if key in cached_2d_windows:
        ...
        wind = np.expand_dims(np.expand_dims(wind, 1), 1)  # 3, 3 to 1, 1
...
def _windowed_subdivs(padded_img,
                      window_size, subdivisions, nb_classes, pred_func):
    """
    ...
    print('######################', subdivs.shape)
    print('type', type(subdivs))
    subdivs = subdivs.reshape(a * b, c, d, e)
    subdivs = np.transpose(subdivs, axes=[0, 3, 1, 2])
    subdivs = torch.Tensor(subdivs)
    print('type', type(subdivs))
    gc.collect()
    ...
    print('######################', subdivs.shape)
    gc.collect()
    subdivs = np.array([patch.detach().numpy() * WINDOW_SPLINE_2D for patch in subdivs])
    print('######################', subdivs.shape)
    gc.collect()
...
def _recreate_from_subdivs(subdivs,
                           window_size, subdivisions, padded_out_shape):
    """
...
def predict_img_with_smooth_windowing(input_img, window_size,
                                      subdivisions, nb_classes, pred_func):
    """
