...
for ds_nm in datasets.keys():
    ...
    class ThresholdModel(nn.Module):
        ...
        def __init__(self, t=0.5, sigmas=range(1, 20, 4), channel_idx=0):
            super().__init__()
            self.t = float(t)
            self.sigmas = sigmas
        ...
        def forward(self, x):
            ...
            for i in range(B):
                ...
                sato_img = sato(img1, sigmas=self.sigmas)
                mask = (sato_img > self.t).astype(np.uint8)
    ...
    class Estimator(BaseEstimator):
        ...
        def __init__(self, threshold=0.5, sigmas=range(1, 20, 4)):
        ...
        def fit(self, X, y=None):
            ...
            self.model = ThresholdModel(t=self.threshold, sigmas=self.sigmas).cpu()
    ...
    model = ThresholdModel(t=bp.get('threshold', 0.5),
                           sigmas=bp.get('sigmas', range(1, 20, 4))).cpu()
    ...
    with open(f'CV_outputs/best_{ALGO}_{ds_nm}_{timestamp}.json', 'w') as fp:
        grid.best_params_["sigmas"] = str(grid.best_params_["sigmas"])
