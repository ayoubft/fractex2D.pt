...
class OVAS(Dataset):
    ...
    def __init__(self, subset: str, list='list.txt', topo=False, transform=False,
        ...
        if list:
            self.fnames = []
            with open(os.path.join(directory, subset, list), 'r') as f:
                # print('USE ONLY THE FRAC SIM METRCIS ON REDUCED TEST DATASET')  # if `list2.txt` is used
                for line in f:
            ...
            self.images = sorted(
                [file.replace(ext_, ext_img)
                    for file in [os.path.join(directory, subset, 'image', fname)
                                 for fname in self.fnames if fname.endswith(ext_)]
                 ])
            self.masks = sorted(
    ...
    def __getitem__(self, index):
        ...
        # return image_tensor.float(), mask_tensor.float(), filename


class MATTEO(Dataset):

    def __init__(self, subset: str, list="list.txt", topo=False, transform=None,
                 expand=True, dilate=True, in_channels=3):
        ...
        if list:
            list_path = os.path.join(directory, subset, list)
            # print('USE ONLY THE FRAC SIM METRCIS ON REDUCED TEST DATASET')  # if `list2.txt` is used
        ...
        self.images = sorted([
            os.path.join(directory, subset, 'image', f"{name}")
            for name in names
        ])

        self.masks = sorted([
            os.path.join(directory, subset, 'gt', f"{name}")
            for name in names
        ])
    ...
    def __getitem__(self, index):
        ...
        dem_tensor = torch.from_numpy(image[:, :, 3]).float()
        # dem_tensor = (dem_tensor - dem_tensor.mean()) / (dem_tensor.std() + 1e-8)
        dem_tensor = (dem_tensor - dem_tensor.min()) / (dem_tensor.max() - dem_tensor.min() + 1e-8)
        mask_tensor = torch.from_numpy(gt).unsqueeze(0) / 255.
...
class GeoCrack(Dataset):
    ...
    def __init__(self, subset: str, list=True, topo=False, transform=None,
                 expand=True, dilate=True, in_channels=3):
